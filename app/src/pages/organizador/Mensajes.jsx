...
import { CONVERSACIONES, ESTADO_MSG } from '../../data/mock'
...
export default function Mensajes() {
  ...
  usePageHeader('Mensajes', `${CONVERSACIONES.length} conversaciones`)
  const conv = CONVERSACIONES.find((c) => c.id === activa)
  const lista = CONVERSACIONES.filter((c) =>
    (filtro === 'Todos' || c.estado === filtro) && c.de.toLowerCase().includes(q.toLowerCase()),
  )
        ...
        <aside className={cls('w-full shrink-0 flex-col border-r border-gray-200 lg:flex lg:w-80', vista === 'hilo' ? 'hidden lg:flex' : 'flex')}>
          <div className="border-b border-gray-200 p-3">
            <div className="relative">
              ...
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar conversación…" className="h-9 w-full rounded-lg border border-gray-200 bg-white pl-9 pr-3 text-[13px] text-ink-body placeholder:text-ink-subtle focus:border-navy focus:outline-none focus:ring-1 focus:ring-mint/40" />
            </div>
            {/* Chips de filtro */}
              ...
              <button key={c.id} onClick={() => { setActiva(c.id); setVista('hilo') }} className={cls('flex w-full items-start gap-3 border-b border-gray-100 px-4 py-3 text-left transition-colors', c.id === activa ? 'bg-mint/10' : 'hover:bg-gray-50')}>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <p className="truncate text-[13px] font-semibold text-ink-strong">{c.de}</p>
                    <span className="shrink-0 text-[10px] text-ink-subtle">{c.hora}</span>
                  </div>
                  <div className="mt-1 flex items-center gap-1.5">
                    <span className="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] font-semibold text-ink-muted">{c.categoria}</span>
                    <Badge tone={ESTADO_MSG[c.estado]}>{c.estado}</Badge>
                  </div>
        ...
        <section className={cls('min-w-0 flex-1 flex-col bg-offwhite lg:flex', vista === 'lista' ? 'hidden lg:flex' : 'flex')}>
          <div className="flex items-center justify-between border-b border-gray-200 bg-white px-4 py-3 sm:px-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2"><p className="text-[13px] font-bold text-ink-strong">{conv.de}</p><Badge tone={ESTADO_MSG[conv.estado]}>{conv.estado}</Badge></div>
                <p className="text-[11px] text-ink-subtle">{conv.categoria} · En línea</p>
              </div>
            </div>
